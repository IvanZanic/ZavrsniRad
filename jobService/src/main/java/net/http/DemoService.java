...
//	@Scheduled(fixedRate= 1000 * 60 * 60 * 24)
	public void getJobsTask() {
		...
		while (!isLastPage) {
			...
			try {
				response = HttpRequestHandling.sendGet(
						"http://www.moj-posao.net/Pretraga-Poslova/",
						urlParameters);
			} catch (IOException e) {
				log.error("IOException: " + e.toString() + ". Parameters: "
						+ urlParameters);
				e.printStackTrace();
			}
		...
		for (Job job : allJobs) {
			...
			if (jobDaoManager.getJobById(job.getId()) == null) {
				...
				if (job.getEmployer().getId() == 0
						|| job.getEmployer().getId() >= 10000000) {
					employer = employerDaoManager.getEmployerByName(job
							.getEmployer().getName());
					...
					if (employer != null) {
						...
						do {
							int employerId = Utils.generateRandomNumber(
									10000000, 99999999);
							employer = employerDaoManager
									.getEmployerById(employerId);
							if (employer == null) {
								job.getEmployer().setId(employerId);
								employerDaoManager.saveEmployer(job
										.getEmployer());
								break;
							}
						} while (employer != null);
					...
					employer = employerDaoManager.getEmployerById(job
							.getEmployer().getId());
					...
					if (employer != null) {
						...
						try {
							response = HttpRequestHandling.sendGet(job
									.getEmployer().getLink(), "");
						} catch (IOException e) {
				...
				try {
					...
					log.error("IOException: " + e.toString() + ". Job link: "
							+ job.getLink());
					e.printStackTrace();
				}
				...
				if (deadline == null) {
					log.error("Deadline error = no deadline, job_id: "
							+ job.getId());
					continue;
				}
